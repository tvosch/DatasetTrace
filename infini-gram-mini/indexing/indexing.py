    ...
    if path.endswith(".gz"):
        ...
        with open(path, "rb") as fh:
            ...
            yield from itertools.islice(
                io.TextIOWrapper(reader, encoding="utf-8"), start_line, stop
            )
...
def _parse_record(
    ...
    meta = (
        json.dumps({"path": rel_path, "linenum": linenum, "metadata": record}) + "\n"
    ).encode("utf-8")
    return data, meta
            ...
            for line in _load_file_slice(path, start_line, end_line):
                ...
                if len(batch) >= batch_size:
                    ...
                    for data, meta in parsed:
                        ds_fout.write(data)
                        od_fout.write(od.to_bytes(8, "little"))
                        od += len(data)
                        mt_fout.write(meta)
                        om_fout.write(om.to_bytes(8, "little"))
                        om += len(meta)
                    del parsed, batch
                    batch = []
                    gc.collect()
...
def _map_worker(
    ...
    with (
        ...
        for linenum, line in enumerate(
            _load_file_slice(path, start_line, end_line), start_line
        ):
...
def _prepare_many(
    ...
    with mp.get_context("fork").Pool(cpus) as pool:
        pool.starmap(
            ...
            [
                (
                    ...
                    doc_sep,
                    text_key,
                    os.path.relpath(path, common_dir),
                )
        ...
        data_sizes = [
            os.path.getsize(tmp / f"text_data.{i:04d}") for i in range(len(files))
        ]
        meta_sizes = [
            os.path.getsize(tmp / f"text_meta.{i:04d}") for i in range(len(files))
        ]
        off_sizes = [
            os.path.getsize(tmp / f"data_offset.{i:04d}") for i in range(len(files))
        ]
        total_data, total_meta, total_off = (
            sum(data_sizes),
            sum(meta_sizes),
            sum(off_sizes),
        )
...
def prepare(
    ...
    outputs = [
        save / n
        for n in ("text_data.sdsl", "data_offset", "text_meta.sdsl", "meta_offset")
    ]
...
def build_sa_bwt(
    save_dir: str, temp_dir: str, mem_gib: int, cpus: int, mode: str
) -> None:
    ...
    log.info(
        "  corpus=%d B, ratio=%d, %d batch(es) x %d job(s)",
        ds_size,
        ratio,
        num_batches,
        cpus,
    ...
    for batch_start in range(0, total_jobs, cpus):
        ...
        for proc in procs:
            ...
            if proc.returncode != 0:
                raise RuntimeError(
                    f"rust_indexing make-part failed:\n{stderr.decode()}"
                )
...
def count_shards(files: list[tuple[str, int]], num_shards: int) -> int:
    ...
    n_slices = sum(
        math.ceil(size / target) if size > target else 1 for _, size in files
    )
...
def split_into_shards(
    ...
    for path, size in files:
        if size <= target:
            ...
            log.info(
                "  Splitting %s into %d pieces (%.1f GiB each)",
                os.path.basename(path),
                n_pieces,
                size / n_pieces / 1024**3,
            )
            ...
            for i in range(n_pieces):
                ...
                slices.append(
                    (path, start, end, int(size * (end - start) / line_count))
                )
...
def index_shard(
    shard_files: list[tuple[str, int, int]],
    common_dir: str,
    save_dir: str,
    temp_dir: str,
...
def main() -> None:
    ...
    parser.add_argument(
        "--num_shards", type=int, default=1, help="Number of index shards to create."
    )
    parser.add_argument(
        "--shard_id",
        type=int,
    ...
    parser.add_argument(
        "--cpus", type=int, default=mp.cpu_count(), help="Number of CPU cores to use."
    )
    parser.add_argument(
        "--mem",
        type=int,
    ...
    try:
        ...
        raise ValueError(
            f"--doc_sep must be a two-character hex string (e.g. 'ff'), got {args.doc_sep!r}"
        )
    ...
    log.info(
        "Found %d source file(s), total %.2f GiB.",
        len(all_files),
        sum(s for _, s in all_files) / 1024**3,
    )
    ...
    for sid in shard_ids:
        if sid >= len(shards):
            log.info(
                "Shard %02d: no files assigned (only %d shard(s) total), skipping.",
                sid,
                len(shards),
            )
            continue
        ...
        if (Path(shard_save) / "data.fm9").exists() and (
            Path(shard_save) / "meta.fm9"
        ).exists():
